...
import { validateForm } from './CreateListing';
...
export default function EditListing() {
    ...
    const uploadImages = async () => {
        ...
        let tempResp = imageUrls.filter(img => typeof img == 'string');

        const uploadPromises = imageUrls.filter(img => typeof img != 'string').map(image => {
        ...
        try {
            ...
            if (responseUrls.length == 0) return tempResp;

            return [...tempResp, ...responseUrls];
        } catch (err) {
    ...
    const handleUpdate = async (evt) => {
        ...
        //validate form
        const { isInvalid, message } = validateForm(formData, imageUrls)
        if (isInvalid) {
            toast.error(message);
            return
        }
        ...
        try {
            ...
            const { id, lat, lng, ...rest } = formData;
            const listingObj = {
                ...rest,
                imageUrls: [...imgUrls],
                geolocation: { lat, lng }
            ...
            await updateDoc(doc(db, 'listings', id), listingObj);

            toast.success("Listing updated successfully")
            for (let i = 0; i < imagesToDisplay.length; i++) {
                const imageUrl = imagesToDisplay[i];
                if (typeof imageUrl === "string" && imageUrl.startsWith("blob:")) {
                    URL.revokeObjectURL(imageUrl); // only revoke blob URLs
                }
            }
            navigate(`/categories/${listingObj.type}/${id}`)
        } catch (error) {
    ...
    useEffect(() => {
        ...
        // send user to profile is listing id is missing from url
        if (!listingId) {
            navigate('/profile');
            return;
        }
        ...
        const fetchDoc = async () => {
            try {
                ...
                // if fetch returns 404, navigate to profile
                if (!docSnap.exists()) {
                    toast.info("Record doesn't exist.")
                    navigate('/profile');
                    return;
                }

                const { id, userId, timestamp, geolocation: { lat, lng }, ...rest } = { id: docSnap.id, ...(docSnap.data()) }

                // user doesnt own record, navigate home
                if (userId !== authUser.uid) {
                    toast.error('Attempted action is forbidden');
                    navigate('/')
                }

                setFormData({ id, userId, lat, lng, ...rest });
                setAddress(rest.location)
            } catch (error) {
    ...
    return !authUser ? null : (
        formData
                    ...
                    <div className="mt-8 relative">
                        {
                            updating
                                ? <div className="absolute inset-0 flex items-center justify-center z-10">
                                    <ImSpinner3 size={32} className='text-blue-600 animate-spin' />
                                </div>
                                : <form onSubmit={handleUpdate}>
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <input onChange={(evt) => {
                                            ...
                                            const newImages = validImages.length > 5 ? validImages.filter((_, idx) => idx < 5) : validImages
                                                        ...
                                                        <div key={idx} className="flex flex-col items-center gap-1">
                                                            ...
                                                            <button
                                                                onClick={() => {
                                                                    ...
                                                                    setFormData(prev => ({ ...prev, imageUrls: prev.imageUrls.filter((_, index) => index != idx) }))
                                                                }}
                                                                type='button'
            ...
            : <div className='w-full h-56 md:h-72 lg:h-96 flex items-center justify-center'>
